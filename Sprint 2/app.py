...
class Round:
    ...
    def __init__(self):
        ...
        self.waiting_for_bid = True #Need a bid from the player to start round
...
def start_new_round():
    deck.round.roundNumber += 1
    deck.round.lead_suit = None
    deck.round.current_trick = []
    deck.player1.bid = None
    deck.round.waiting_for_bid = True
...
def home():
    ...
    context["deck"] = deck
    return render_template("game.html", round=deck.round, **context)
...
def set_bid():
    ...
    deck.round.waiting_for_bid = False
    return redirect("/game")
